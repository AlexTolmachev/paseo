...
The only built-in ACP provider today is `copilot` (`copilot-acp-agent.ts`). `GenericACPAgentClient` (`generic-acp-agent.ts`) is also ACP-based but is used for user-defined custom providers configured via `extends: "acp"` overrides — see [docs/custom-providers.md](custom-providers.md). Catalog `grok` is the same `extends: "acp"` shape, but the registry routes it to `GrokACPAgentClient` because Grok's occupancy, compact notifications, and conversation rewind are vendor ACP, not the generic protocol.
